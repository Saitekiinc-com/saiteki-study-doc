...
本システムで使用しているAPIと、想定されるランニングコストの目安です。（社員10名が毎月利用した場合の試算）

| 項目 | 使用モデル/API | 単価 (目安・Stable版想定) | 想定月額コスト |
| :--- | :--- | :--- | :--- |
| **テキスト生成** | Gemini 3 Flash Preview | 入力: $0.50/1M token<br>出力: $3.00/1M token | **約 $0.05 (約7円)**<br><small>※Preview期間中は無料の場合あり</small> |
| **ベクトル化** | Embedding-001 | $0.0000025 / 1k token | **$0.01未満**<br><small>※感想文の蓄積量に依存</small> |
| **書籍検索** | Google Books API | **無料** (Public) | **$0** |
| **実行環境** | GitHub Actions | 無料枠内 (Public Repos) | **$0** |

**合計予測**: 月間 **$1.00 (約150円) 以下** で運用可能です。
